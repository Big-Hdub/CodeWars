...
def tribonacci(signature, n):
    """
    Generates the first n elements of a Tribonacci sequence.

    A Tribonacci sequence is similar to a Fibonacci sequence, but instead of
    starting with two predetermined terms, it starts with three predetermined
    terms and each subsequent term is the sum of the preceding three terms.

    Parameters:
    signature (list): A list of three numbers that starts the Tribonacci sequence.
    ...
    list: A list containing the first n elements of the Tribonacci sequence.
          If n is 0, an empty list is returned. If n is less than or equal to 3,
          the function returns the first n elements of the signature.
    """
    ...
    # Initialize the result list with the signature
    res = [*signature]

    # Generate the remaining elements of the sequence
    for _ in range(n - 3):
        # Append the sum of the last three elements to the result list
        res.append(sum(res[-3:]))

    # Return the result list containing the first n elements of the Tribonacci sequence
    return res
